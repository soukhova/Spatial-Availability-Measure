...
In this section we calculate _job access_ using both accessibility and spatial availability measures for jobs in city of Toronto and all origins in the GGH using the impedance function defined in the previous section. Toronto is the largest city in the GGH and represents a significant subset of workers and jobs in the GGH; `r percent(sum(TO_od_ft$workers)/sum(od_ft$workers))` of workers in the GGH travel to jobs in Toronto and  `r percent(sum(TO_od_ft$jobs)/sum(od_ft$jobs))` of jobs are located within Toronto. As it will be later discussed, this example demonstrates the first issue in accessibility: neglecting to include the single-constraint overestimates _job access_ for TAZ with a low proportion of workers that are next to TAZs with a high proportion of jobs and workers. 
...
In this section we calculate job access for all jobs in the GGH for all origins in the GGH using both accessibility and spatial availability measures. As will be later elaborated, this example demonstrates the second issue in the accessibility measure: neglecting to include the single-constraint underestimates job access for TAZ which are on the periphery of the GGH and have relatively-low proportion of workers. These TAZs are relatively isolated (by travel cost) from the city of Toronto which has the highest density of opportunities (jobs). The first issue, namely the over estimation of the TAZs which are on the outer limits outside the Toronto border is also observed.
...
Spatial availability within this example can also be represented on a per-worker basis and a similar difference in trend between the previous plot in Figure \ref{fig:plot-avail-Toronto-TTS-per-worker} can be seen in the plot for all GGH jobs in Figure \ref{fig:plot-avail-GGH-TTS-per-worker}.
...
We have used both accessibility and spatial availability to measure access to job and have compared trends superficially for the empirical GGH data set; a thorough comparative discussion of this data set is detailed in this section. 

## Comparing access to jobs 
...
indexed_measures_ALL <- TO_taz_acc %>%
...
indexed_measures_pos <- indexed_measures_ALL %>%
  filter(A_V_indexed_change >= 0 ) 

indexed_measures_neg <- indexed_measures_ALL %>%
  filter(A_V_indexed_change < 0 )#& (
...
boxplot(indexed_measures_pos$A_V_indexed_change, horizontal=TRUE)
boxplot(indexed_measures_neg$A_V_indexed_change, horizontal=TRUE)
boxplot(indexed_measures_ALL$A_V_indexed_change, horizontal=TRUE)

# as a comment.. Moran I's test is not robust to outliers; so I  removed Q5 from the negative change data set (i.e. change which represents -70% to 0, only changes which are greater remain )
```


```{r weights-matrix-for-moran, include=FALSE}
taz_nb <- poly2nb(indexed_measures_neg, queen = TRUE)
taz_w <- nb2listw(taz_nb, zero.policy = TRUE)
```
```{r moran-test-access-change, include=FALSE}
moran.test(indexed_measures_neg$A_V_indexed_change, taz_w, zero.policy = TRUE, na.action=na.exclude)
```

```{r local-moran-i, include=FALSE}
local_i <- localmoran(indexed_measures_neg$A_V_indexed_change,
                      taz_w, 
...
indexed_measures_neg$significance <- local_i$significance
#graphinh histogram to create good breaks for plot
hist(indexed_measures_neg$A_V_indexed_change, breaks = 4)
```

```{r plot-local-i-Toronto, echo=FALSE, fig.cap="\\label{fig:plot-local-i-Toronto}Difference in re-scaled accessibility and spatial availability job access measure in the context of employment from origins in the GGH to destinations in the City of Toronto. Vivid TAZs with black borders represent a significant spatial autocorrelation  and dimmed TAZs with grey borders represent a non-signficant autocorrelation as measured by Local Moran's I." }

plot_morani_Toronto <- ggplot() +
  geom_sf(data = indexed_measures_neg,
          aes(fill = A_V_indexed_change,
              color = significance,
              alpha = significance)) +
  geom_sf(data = toronto_muni_bound, # border for Toronto
          colour=alpha("dimgrey",1), 
  ...
  scale_fill_distiller(palette = "Spectral",
                       ...
                       labels = c("-100% to -80%",
                                  ...
                                  "-39% to -20%",
                                  "-19% to -2%")) +
  scale_color_manual(values = c("n.s." = "gray", 
                                "sig" = "black")) +
  scale_alpha_manual(values = c("n.s." = 0.7, 
                                "sig" = 1)) +
  guides(color = FALSE, 
         alpha = FALSE,
