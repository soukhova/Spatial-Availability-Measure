...
```{r data-for-Toronto-boundaries, include=FALSE}
...
Toronto_highways <- st_read("data-inputs/fToronto_highways.shp")
...
- Conventional accessibility measures

- An alternative approach: **spatial availability**

- Toronto full-time job availability example

- Potential applications  
  - Benchmarking: jobs per person     
  - Targeting improvements  
  - Quantifying scenario impacts  
 
- Limitations - the input data
...
$V_{i} = \sum_{j} O_j F_{ij}$

- The proportional allocation factor combines two aspects of the problem, population and cost:

$F_{ij} = \frac{F^p_{i} \cdot F^c_{ij}}{\sum_{i} F^p_{i} \cdot F^c_{ij}}$

- Where $F^p_{ij}$ is population allocation factor:

$F^p_{ij} = \frac{P_{i}}{\sum_{i} P_{i}}$

- And the $F^c_{ij}$ is a cost allocation factor:
...
TO_taz_acc %>% mutate(Ai = S_i, Vi = V_i) %>% dplyr::select(Ai, Vi) %>% tidyr::pivot_longer(cols=-geometry, names_to = "Measure", values_to = "Values") %>%
  ggplot() + 
...
TO_taz_acc %>% mutate(jwork = ifelse(jwork >= 1.87, 1.87, jwork),
  ...
  mutate(`Jobs by worker` = jwork , vi = v_i) %>% dplyr::select(`Jobs by worker`, vi) %>% 
...
- Singly-constrained accessibility reinterpretation of 2SFCA

- Solves logical inconsistencies with Shen-style accessibility
...
#### Paper is open access: \url{https://doi.org/10.1371/journal.pone.0278468}
