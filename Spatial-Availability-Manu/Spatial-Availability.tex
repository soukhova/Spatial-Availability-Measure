...
availability, can also be used as-is to evaluate the spatial mismatch of
accessible opportunities and demand-seeking population within regions
and between regions.
...
Considering both population and cost allocation factors in \(V_{ij}\),
the sum of jobs at employment center \(O_1\) that are allocated to
population centers \(P_1\) and \(P_2\) are still preserved (i.e.,
\(V_{1,1} + V_{2,1} = O_1\)). Notice that \(V_{ij}\), allocates a number
of jobs to \(P_1\) and \(P_2\) which is between the values allocated in
\(V^p_{ij}\) and \(V^c_{ij}\).
...
centers is \(\frac{300}{240 + 120}=\) 0.83.
...
and includes the cities of Toronto and Hamilton. We calculate spatial
accessibility and availability for the GGH after introducing the data
...
(\ref{eq:dif-index-measures}). This ratio represents how many times
over- or under- estimated job access is for a TAZ when measured by the
...
to conventional accessibility as it provides more meaningful estimates
of opportunity access by ensuring the estimates match the observed
number of opportunities. Spatial availability enables the calculation of
per capita measures and measures relative to regional averages since all
opportunities are preserved. This development makes it easier to compare
opportunity landscapes across regions and/or time.
...
This comparison distinguishes how the measure's proportional allocation
of opportunity impacts the access landscape. Fundamentally, the
traditional accessibility measure's methodology results in the
...
non-divisible opportunities (i.e., only one person can occupy one seat
of the opportunity at any given time). Though we focus on employment in
this paper, opportunity access to any competitive opportunity such as
schools, hospitals, and other essential services with finite capacity at
a given time can be measured using \emph{spatial availability}.
Additionally, opportunities that are non-competitive, can still benefit
...
\emph{availability} perspective through the reduction in competition
effect (from accessibility) and a meaningful benchmark of demand per
population unit.
