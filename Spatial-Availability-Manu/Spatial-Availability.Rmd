...
V_{2,1} = O_1\cdot \frac{F^p_{2,1} \cdot F^c_{2,1}}{F^p_{1,1} \cdot F^c_{1,1} + F^p_{2,1} \cdot F^c_{2,1}} = 300 \frac{\big(\frac{1}{3} \big) \big(0.574 \big)}{\big(\frac{2}{3} \big) \big(0.426 \big) + \big(\frac{1}{3} \big) \big(0.574 \big)}  =  120.6 \\
\end{array}
\end{equation}
```
Considering both population and cost allocation factors in $V_{ij}$, the sum of jobs at employment center $O_1$ that are allocated to population centers $P_1$ and $P_2$ are still preserved (i.e., $V_{1,1} + V_{2,1} = O_1$). Notice that $V_{ij}$, allocates a number of jobs to $P_1$ and $P_2$ which is between the values allocated in $V^p_{ij}$ and $V^c_{ij}$.
...
In this paper, we introduce a singly-constrained accessibility measure that we term *spatial availability*. This measure is an alternative to conventional accessibility as it provides more meaningful estimates of opportunity access by ensuring the estimates match the observed number of opportunities. Spatial availability enables the calculation of per capita measures and measures relative to regional averages since all opportunities are preserved. This development makes it easier to compare opportunity landscapes across regions and/or time. 
...
This comparison distinguishes how the measure's proportional allocation of opportunity impacts the access landscape. Fundamentally, the traditional accessibility measure's methodology results in the overestimation or underestimation of opportunity access as a result of the competition effect. Accessibility does not include factors to bound the summation, so origins, hypothetically, can have infinite opportunity access. By including an intuitive single-sided constraint in the new spatial availability measure, it can be used to calculate access to non-divisible opportunities (i.e., only one person can occupy one seat of the opportunity at any given time). Though we focus on employment in this paper, opportunity access to any competitive opportunity such as schools, hospitals, and other essential services with finite capacity at a given time can be measured using  _spatial availability_. Additionally, opportunities that are non-competitive, can still benefit from this measure as access will not be over- or under-estimated as a result of the competition effect seen in accessibility; non-competitive opportunities can include large natural parks and beaches. As such, values measured for opportunities of any type can benefit from an _availability_ perspective through the reduction in competition effect (from accessibility) and a meaningful benchmark of demand per population unit.
