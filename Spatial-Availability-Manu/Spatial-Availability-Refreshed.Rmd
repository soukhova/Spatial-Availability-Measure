...
In this section we illustrate the application of spatial availability through an empirical example. For this, we use population and employment data from the Greater Golden Horseshoe (GGH) area in Ontario, Canada. Contained with the GGH is the Greater Toronto and Hamilton (GTHA) which form the most populous metropolitan regions in Canada and the core urban agglomeration in the GGH. 
...
```{r data-for-impedance-Toronto, include=FALSE}
TO_od_ft  <- od_ft_tt %>% 
...
TO_taz_jobs <- TO_taz %>% mutate(jobs = ifelse(jobs >= 10000, 10000, jobs),
  ...
  geom_sf(aes(fill = jobs), col = "grey90", size=0.1) +
     scale_fill_distiller(name = "Jobs",
                       ...
                       direction = 1,
                       na.value = "white",
                       label = c( round(min(TO_taz$jobs, na.rm=T)), "305 (1st Qu.)", "657 (median)", "1706 (3rd Qu.)", "10000 to 41821"), 
                       breaks = c( round(min(TO_taz$jobs, na.rm=T)), 305, 657, 1706, 10000)) + 
...
TO_taz_workers <- TO_taz %>% mutate(workers = ifelse(workers == 0, NA, workers)) %>% 
  ggplot() +
  ...
  scale_fill_distiller(name = "Workers",
                       ...
                       direction = 1,
                       na.value = "white",
                       label = c( round(min(TO_taz$workers, na.rm=T)), "1044 (1st Qu.)", "1640 (median)", "2556 (3rd Qu.)", round(max(TO_taz$workers, na.rm=T))), 
                       breaks = c( round(min(TO_taz$workers, na.rm=T)), 1044, 1640, 2556, round(max(TO_taz$workers, na.rm=T)))) + 
...
TO_taz_jobs_workers <- TO_taz %>% mutate(jwork = ifelse(jwork >= 3, 3, jwork),
  ...
  scale_fill_distiller(name = "Jobs:workers",
                       ...
                       direction = 1,
                       na.value = "white",
                       label = c( round(min(TO_taz$jwork, na.rm=T)), "0.119 (1st Qu.)", "0.297 (median)", "0.747 (3rd Qu.)", "3 to 2760"),
                       breaks = c( round(min(TO_taz$jwork, na.rm=T)), 0.119, 0.297, 0.747, 3)) +
  annotation_north_arrow(location = "tr", # north arrow for both the main plot and inset
                         height = unit(0.5, "cm"), 
                         width = unit(0.5, "cm"),
                         style = north_arrow_orienteering(line_width = 0.15,
...
TO_taz_tt <- TO_taz %>% mutate(workers = ifelse(tt_mean == 0, NA, tt_mean)) %>% 
  ggplot() +
...
<!-- NOTE: AP, maybe this is worth explaining more..? or is it fine - I think it's okay but may be worth some additional discussion/relations to toronto-specific literature -->
As mentioned, the focus of this empirical example is on the city of Toronto. It is the largest city in the GGH and represents a significant subset of workers and jobs in the GGH; `r percent(sum(TO_taz$workers)/sum(TTS2016R::ggh_taz$workers))` of workers in the GGH travel live in Toronto and `r percent(sum(TO_taz$jobs)/sum(TTS2016R::ggh_taz$jobs))` of jobs are located within Toronto. The spatial distribution of jobs (purple) and workers (orange) are captured in Figure \ref{fig:s-dist-Toronto-plot}. It can be seen that a large cluster of jobs can be found in the central southern part of Toronto (the downtown core). Spatial trends in the distribution of workers is less apparent relative to the distribution of jobs.  
...
With spatial availability, we can push accessibility analysis forward by making it more interpretable
...
Toronto is the largest city in the GGH and represents a significant subset of workers and jobs in the GTHA; `r percent(sum(TO_workers$workers)/sum(TTS2016R::ggh_taz$workers))` of workers in the GTHA travel to jobs in Toronto and `r percent(sum(TO_jobs$jobs)/sum(TTS2016R::ggh_taz$jobs))` of jobs are located within Toronto. 
...
- Hansen-style accessibilty should be used when opportunities are non-exclusive. When opportunities are perfectly exclusive (i.e., 1 spot for 1 person), spatial availability (i.e., accessibility with competition) should be used.
...
This can be plainly seen by seeing the proportion of population, in spatial availability, which is allocated to the origin <!-- uh... fix this -->
